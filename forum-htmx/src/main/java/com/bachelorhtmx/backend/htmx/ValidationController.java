...
public class ValidationController {
    ...
    @PostMapping(value = "/register/new", consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.TEXT_HTML_VALUE)
    public String registerNewUser(Model model, @RequestParam Map<String, String> body, HttpServletResponse response) {
        String username = body.get("username");
        String displayname = body.get("displayname");
        String password = body.get("password");
        User user = userService.getUserByUsername(username);

        String jwt = authenticationService.register(username, displayname, password);
